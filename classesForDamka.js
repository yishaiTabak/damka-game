...
export class Pawn {
    ...
    //          this.isCaptureMove(locationRow, locationColumn, destinationRow, destinationColumn, pieces)
    // }
    ...
    itHasOptionToMove(locationRow, locationColumn, pieces) {
        for(let i = 0;i<8;i++)
            for(let j = 0; j<8;j++)
                if(this.isRegularMove(pieces, locationRow, locationColumn, i, j) ||
                 this.isCaptureMove(locationRow, locationColumn, i, j, pieces))
                    return true
        return false
    }
    isLegalMove(locationRow, locationColumn, destinationRow, destinationColumn, isWhiteTurn, pieces) {
        ...
        let optionsToCapture = []
        this.isOptionToCapture = false
        let newPieces = CheckersGame.newPieces(pieces)
        this.bbb(newPieces, locationRow, locationColumn, optionsToCapture,destinationRow, destinationColumn, true)
        // for(let option of optionsToCapture)
        //     if(option[0] === destinationRow && option[1] === destinationColumn)
        //         return true
        if(this.isOptionToCapture)
            return true
        return this.isRegularMove(pieces, locationRow, locationColumn, destinationRow, destinationColumn)
    }
    bbb(newPieces,locationRow, locationColumn, optionsToCapture,destinationRow, destinationColumn, isFirsCapture) {
        let isTheLast = true
            ...
            if(isIndexesInBoard)
                if(isFirsCapture) {
                    if(this.isCaptureMove(locationRow, locationColumn, newRow, newColumn, newPieces)) {
                        newPieces = CheckersGame.handleCaptureMove(newPieces, locationRow, locationColumn, newRow, newColumn)
                        this.bbb(newPieces, newRow, newColumn, optionsToCapture,destinationRow, destinationColumn, false)
                        isTheLast = false
                    }
                }
        ...
        // if(isTheLast &&locationRow === destinationRow && locationColumn === destinationColumn)
        //     optionsToCapture.push([locationRow, locationColumn])
    }
 }
...
//     constructor(board,locationRow, locationColumn)
// }